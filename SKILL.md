...
description: "维护、审查、重构、现代化或调试 Objective-C iOS 项目。用于 .h/.m/.mm、UIKit、ARC 所有权、block/retain cycle、NSError、线程与异步、CoreFoundation bridge、KVC/KVO、runtime/category/swizzling、Swift 混编、Xcode 工程、project.pbxproj、target/scheme、xcconfig、CocoaPods、SPM、静态库、闭源 .a、xcframework、-ObjC、-force_load、category 符号裁剪、头文件/PCH/modulemap、编译速度、Auto Layout、列表滚动、UIKit 渲染、启动、pre-main、动态库数量、内存、崩溃、crash log、崩溃日志符号化、dSYM、MetricKit、MXCrashDiagnostic、MXHangDiagnostic、EXC_BAD_ACCESS、野指针、Zombie、ASan、Malloc Scribble、Guard Malloc、OOM、Jetsam、FOOM、watchdog、0x8badf00d、网络缓存、性能诊断、静态风险巡检、新手 OC + UIKit 架构和从零编写保守安全代码。"
---
...
18. 如果涉及 Xcode 工程、`project.pbxproj`、target/scheme、`.xcconfig`、CocoaPods、SPM、静态库、闭源 `.a`、`xcframework`、`-ObjC`、`-force_load`、category 符号裁剪、头文件依赖、PCH、umbrella header、modulemap 或编译速度，读 `references/build-system-dependencies.md`。
...
- category 位于静态库/Pod/闭源 `.a`，但最终 App/Extension target 没有继承 `-ObjC`，线上触发 `unrecognized selector`。
- 用 `-all_load` 掩盖缺失 category 符号，后续引入重复符号和体积膨胀。
- `OTHER_LDFLAGS`、Header/Framework/Library Search Paths 缺少 `$(inherited)`，覆盖 CocoaPods 或上层 xcconfig 配置。
- `project.pbxproj` 合并冲突只删 marker，没有核对 scheme、target、build phase 和 configuration。
- CocoaPods、SPM 和手动二进制重复引入同一个底层库或资源 bundle。
- PCH 引入业务头和重型 SDK，总头或 umbrella header 暴露 private header，导致改一个头全量重编。
...
- 构建系统：`project.pbxproj` 是否无冲突，scheme/target/configuration 是否一致，`.xcconfig` 是否分层且保留 `$(inherited)`？
- 依赖链接：静态库/Pod category 是否有 `-ObjC` 或精确 `-force_load` 证据，是否避免全局 `-all_load`？
- 依赖管理：CocoaPods、SPM、手动二进制和 `xcframework` 是否有清晰 owner、版本、资源、dSYM 和 module 边界？
- 头文件与编译：PCH、umbrella header、modulemap、Header Search Paths 和 script phase 是否导致不必要全量编译？
