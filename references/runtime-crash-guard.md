...
当任务明确要求使用 Objective-C runtime、消息转发、category 或 swizzling 做线上崩溃兜底时读取本文件。本文件是“可选、强警告、最后兜底”方案，不是默认修复路径。若缺少符号化 crash log、dSYM UUID 证据或 MetricKit 辅助诊断，先读取 `references/crash-symbolication-metrickit.md`。
...
| 证据是什么？ | 符号化 crash log、dSYM UUID 匹配、版本分布、崩溃量级、调用栈、MetricKit/业务日志、最近变更和复现可能性。 |
...
遇到 `EXC_BAD_ACCESS`、`objc_msgSend`、Zombie、ASan、Malloc Scribble、Guard Malloc、use-after-free 或 double free 任务时，读取 `references/dangling-pointer-diagnostics.md`，先定位访问栈、分配栈和释放栈，再决定修复点。
...
不要承诺用 Objective-C category 或 runtime guard 兜住 C/C++ 崩溃、OOM、Jetsam、FOOM 或 watchdog。OOM/Jetsam/FOOM/watchdog 诊断读取 `references/oom-watchdog-diagnostics.md`。
